...
def build_analysis_payload(region=None):
    env_history = get_env_history(hours=24)
    soil_history = get_soil_history(hours=24)
    if region:
        env_history = [item for item in env_history if item.get("region") == region]
        soil_history = [item for item in soil_history if item.get("region") == region]

    labels, temp_series = _series_from_history(env_history, "temperature")
    _, humidity_series = _series_from_history(env_history, "humidity")
    _, soil_series = _series_from_history(soil_history, "soil_moisture")
